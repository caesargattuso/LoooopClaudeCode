...
测试统一放在最后阶段执行，而不是分散在各任务中：

- **开发阶段**：专注核心功能实现，不编写测试
- **最终测试阶段**：一个统一的测试任务覆盖所有已完成功能
- **优势**：更快迭代、统一测试覆盖、减少任务碎片化

体现在任务拆解中：
- 不为单个功能创建测试任务
- 最后创建一个统一测试任务，依赖所有功能任务
- 示例："编写全面的测试覆盖所有已完成功能"（依赖ID: 1,2,3,4,5...）

## 任务粒度控制

采用合并策略减少碎片化：

- **目标任务数**：典型项目 3-15 个任务，避免 30+ 个小任务
- **合并原则**：同模块任务合并、连续小任务合并
- **示例**：
  - ✅ "实现用户认证模块"（包含：模型、路由、验证、会话处理）
  - ❌ "创建文件X" / "编写函数Y"（太细碎）
